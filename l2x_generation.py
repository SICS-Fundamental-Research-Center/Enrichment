...
parser.add_argument("--task", type=str, default='persons-3hop')
...
ks['orange_skin'] = main_args.max_path
ks[main_args.data_name] = main_args.max_path
...
# 使用函数创建目录
create_directory_if_not_exists("model/%s" % main_args.data_name)
...
def create_data_modify(datatype):
    if(datatype=='persons'):
        ...
        train = pd.read_csv('data/l2x-similarity/%s_train.csv' % main_args.data_name,index_col=0)
        valid = pd.read_csv('data/l2x-similarity/%s_valid.csv' % main_args.data_name,index_col=0)
        x_train = np.array(train.iloc[:,:-1]).astype(float)
        y_train = np.array(train.iloc[:,-1]).astype(int)
        y_train_minus = 1 - y_train
...
def L2X(datatype, train = True,epoch=1): 
    ...
    if train: 
        ...
        model.compile(loss='categorical_crossentropy',
                      optimizer=adam,
                      metrics=['acc']) 
        filepath="model/{}/L2X.hdf5".format(datatype)
        ...
        model.load_weights('model/{}/L2X.hdf5'.format(datatype), 
            by_name=True) 
...
median_ranks, exp_time, train_time, samples = L2X(datatype, 
        train = True, epoch=main_args.epoch)
...
print(top_indices)
np.save('data/l2x-similarity/%s_path.npy' % main_args.task,top_indices)