...
def inner_train(args: PromptEMArgs, model, optimizer, scaler, train_dataloader, valid_dataloader, test_dataloader,
    ...
    print(f"[Valid] Precision: {valid_p:.4f}, Recall: {valid_r:.4f}, F1: {valid_f1:.4f}")
    if prompt:
        test_p, test_r, test_f1 = eval_prompt(args, model, test_dataloader)
    else:
        test_p, test_r, test_f1 = eval_plm(args, model, test_dataloader)
    print(f"[Test] Precision: {test_p:.4f}, Recall: {test_r:.4f}, F1: {test_f1:.4f}")
...
def train_and_update_best(args: PromptEMArgs, model, optimizer, scaler, train_dataloader, valid_dataloader,
    ...
    metric = inner_train(args, model, optimizer, scaler, train_dataloader, valid_dataloader, test_dataloader,
                             prompt)
    # inner_train(args, model, optimizer, scaler, train_dataloader, valid_dataloader, test_dataloader,
    #                          prompt)
...
def self_training(args: PromptEMArgs, data: PromptEMData):
    ...
    for iter in range(1, args.num_iter + 1):
        ...
        print(f"[Current Train Set] Size: {siz} Pos: {pos} Neg: {neg} Per: {per:.2f} Acc: {acc:.4f}")
        ...
        # print(f"[Best Teacher in iter#{iter}] Precision: {p:.4f}, Recall: {r:.4f}, F1: {f1:.4f}")
        # if args.self_training:
        #     # generate pseudo labels
        #     num = gen_pseudo_labels(args, data, model)
        #     print(f"[Add Pseudo Labels] Size: {num}")
        #     siz, pos, neg, per, acc = statistic_of_current_train_set(data)
        #     print(f"[Current Train Set] Size: {siz} Pos: {pos} Neg: {neg} Per: {per:.2f} Acc: {acc:.4f}")
        ...
        #     scaler = GradScaler()
            ...
            #         train_loader = get_prompt_dataloader(args, train_set, shuffle=True)
        p, r, f1 = best.test_metric
        print(f"[Best in iter#{iter}] Precision: {p:.4f}, Recall: {r:.4f}, F1: {f1:.4f}")
        return model,optimizer,scaler,test_loader,best.test_metric
...
def self_training_update(args: PromptEMArgs, data: PromptEMData, model_input,optimizer_input,scaler_input):
    train_set = read_prompt_dataset(data.left_entities, data.right_entities, data.train_pairs, data.train_y)
    valid_set = read_prompt_dataset(data.left_entities, data.right_entities, data.valid_pairs, data.valid_y)
    test_set = read_prompt_dataset(data.left_entities, data.right_entities, data.test_pairs, data.test_y)
    valid_loader = get_prompt_dataloader(args, valid_set, shuffle=False)
...
def eval_prompt_update(args, model, data_loader, return_acc=False):
    model.eval()
    y_truth_all = []
    y_pred_all = []
    for batch in data_loader:
        ...
        with torch.no_grad():
            ...
            y_truth_all.extend(y_truth.cpu().numpy().tolist())
            y_pred_all.extend(y_pred.cpu().numpy().tolist())
    return np.array(y_truth_all), np.array(y_pred_all)
def ValidDataLoader(args,dataframe,model):
    if not isinstance(dataframe,PromptDataLoader):
        left_entities = dataframe.iloc[:,0].to_list()
        right_entities = dataframe.iloc[:,1].to_list()
        test_pairs = [(i,i) for i in range(len(dataframe))]
        test_y = dataframe.iloc[:,2].to_list()
        test_set = read_prompt_dataset(left_entities, right_entities, test_pairs, test_y)
        test_loader = get_prompt_dataloader(args, test_set, shuffle=False)
    else:
        test_loader = dataframe
    truth,pred = eval_prompt_update(args,model,test_loader)
    return f1_score(y_true=truth,y_pred=pred)
