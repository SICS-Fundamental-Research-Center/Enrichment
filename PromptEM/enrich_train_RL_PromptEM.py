...
from train import self_training, self_training_only_plm, load_model, ValidDataLoader,self_training_update
...
# env_dict = {'persons_SchemaEnr':"Enrich-wiki-PromptEM",
#             'persons_AutoFeature':"Enrich-wiki-dqn-PromptEM",
#             'imdb_SchemaEnr':"Enrich-imdb-PromptEM",
#             'imdb_AutoFeature':"Enrich-imdb-dqn-PromptEM",
#             'amazon-google_SchemaEnr':"Enrich-amazon-PromptEM",
#             'amazon-google_AutoFeature':"Enrich-amazon-dqn-PromptEM",
#             'dblp_SchemaEnr':"Enrich-dblp-PromptEM",
#             'dblp_AutoFeature':"Enrich-dblp-dqn-PromptEM",
#             'imdb_SchemaEnr_1hop':"Enrich-imdb-1hop-PromptEM",
#             'imdb_AutoFeature_1hop':"Enrich-imdb-1hop-dqn-PromptEM",
#             'imdb_SchemaEnr_3hop':"Enrich-imdb-3hop-PromptEM",
#             'imdb_AutoFeature_3hop':"Enrich-imdb-3hop-dqn-PromptEM",
#     }
...
args = SimpleNamespace(num_iter = 1,
                     ...
                     save_model=True,
                     logdir="model_log",
                     model_name_or_path=main_args.lm,
                     fp16=True,
                     add_token=True,
                     dynamic_dataset=-1,
                     ...
                     template_no=0,
                     one_word=False,
                     teacher_epochs=main_args.epoch,
                     student_epochs=10,
                     lr=2e-5,
                     self_training=False,
                     )
...
model_out,optimizer,scaler = load_model(args,main_args.base_model)
model_out = model_out.to(args.device)

for epoch_count in range(10):
    if(main_args.method=='SchemaEnr'): 
        env = gymnasium.make('Enrich-SchemaEnr-PromptEM',mask=mask_array,relation_dict=relation_dict,mutual_info=mutual_info,state_path=state_path,test_data=enrich_valid,data_name=main_args.data_name,feature_num=action_space, max_episode_steps=100,device_id=2,epoch=epoch_count,model_input = model_out,task=task,max_path = max_path ,lm=main_args.lm)
        ...
        env = gymnasium.make('Enrich-AutoFeature-PromptEM',mask=mask_array,relation_dict=relation_dict,mutual_info=mutual_info,state_path=state_path,test_data=enrich_valid,data_name=main_args.data_name,feature_num=action_space, max_episode_steps=100,device_id=2,epoch=epoch_count,model_input = model_out,task=task,max_path = max_path ,lm=main_args.lm)
    ...
    if(main_args.update):
        ...
        prompt_em_data = PromptEMData(update_data,update_data_valid,update_data_test)
        model_out,optimizer,scaler = self_training_update(args, prompt_em_data,model_out,optimizer,scaler)
    print('Epoch_%s Training Result:' % str(epoch_count))
