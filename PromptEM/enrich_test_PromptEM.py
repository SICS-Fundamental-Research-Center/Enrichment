...
from train import self_training, self_training_only_plm, load_model, ValidDataLoader
...
for epoch_count in episode:
    ...
    prompt_em_data = PromptEMData(enrich_base_train_output,enrich_base_valid_output,enrich_base_test_output)
    model_out,optimizer,scaler,test_loader,best = self_training(args, prompt_em_data)
    p, r, f1 = best
    logger.info(f"Best  Precision: {p:.7f}, Recall: {r:.7f}, F1: {f1:.7f}")
    logger.info('episode_%s Task: %s f1-score: %s, path %s' % (str(epoch_count),task,str(f1),str(feature)))
    if(epoch_count==episode[-1]) and main_args.save_model:
        ckpt = {'model': model_out.state_dict(),
        'optimizer': optimizer.state_dict(),
        'scaler': scaler.state_dict()}
        torch.save(ckpt, 'model/%s.ckpt' % str(task))