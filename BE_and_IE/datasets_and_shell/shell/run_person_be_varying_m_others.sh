...
#python ../dataset/proc.py ${baseConfigFile} ${outputConfigFile} 1.0 no 1 1 1.0 1.0 ${nw}
...
python ../dataset/proc.py ${baseConfigFile} ${outputConfigFile} 1.0 no 0 1 1.0 1.0 20 0.1 ${ms}
    #hdfs dfs -rm '/tmp/enrich/'${data[${dataID}]}'/config_run.json'
    #hdfs dfs -put ${outputConfigFile} '/tmp/enrich/'${data[${dataID}]}'/config_run.json'
    #cd /root/project/enrich/discovery/
    cd ..
    ./run_enrich_unit.sh './shell/configAll_others/config_run.json' >> ${outputFilePrefix}'_'${data[${dataID}]}'_m'${ms}'_others.txt'
    cd ./shell/
    #cd /root/project/enrich/discovery/enrich/shell
    # download results    
    #hdfs dfs -get ${outputFilePrefix}'_'${data[${dataID}]}'_m'${ms}'_others.txt' ../results/
